...
class FCOSOutputs(nn.Module):
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges, num_loc_list):
        ...
        for im_i in range(len(targets)):
            ...
            if self.center_sample:
                is_in_boxes = self.get_sample_region(bboxes, self.strides, num_loc_list, xs, ys, radius=self.radius)
            else:
        ...
        return {"labels": labels, "reg_targets": reg_targets, "target_inds": target_inds}
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, gt_instances):
        ...
        return self.fcos_losses(labels, reg_targets, logits_pred, reg_pred, ctrness_pred, gt_inds)
    ...
    def fcos_losses(
        ...
        if pos_inds.numel() > 0:
            reg_loss = self.loc_loss_func(reg_pred, reg_targets, ctrness_targets) / loss_denorm

            ctrness_loss = (
                F.binary_cross_entropy_with_logits(ctrness_pred, ctrness_targets, reduction="sum") / num_pos_avg
            )
        else:
        ...
        extras = {"pos_inds": pos_inds, "gt_inds": gt_inds, "gt_ctr": ctrness_targets, "loss_denorm": loss_denorm}
        return losses, extras

    def predict_proposals(self, top_feats, logits_pred, reg_pred, ctrness_pred, locations, image_sizes):
        if self.training:
        ...
        for i, per_bundle in enumerate(zip(*bundle.values())):
            ...
            sampled_boxes.append(self.forward_for_single_feature_map(l, o, r, c, image_sizes, t))
    ...
    def select_over_all_levels(self, boxlists):
        ...
        for i in range(num_images):
            ...
            if number_of_detections > self.post_nms_topk > 0:
                ...
                image_thresh, _ = torch.kthvalue(cls_scores.cpu(), number_of_detections - self.post_nms_topk + 1)
